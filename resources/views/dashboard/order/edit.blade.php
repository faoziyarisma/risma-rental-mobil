    ...
    <div class="container-fluid isian-db">
        ...
        <div class="card shadow my-4 w-75">
            ...
            <div class="card-body px-lg-3 mt-4">
                <form action="/dashboard/orders/{{ $order->id }}" enctype="multipart/form-data" method="post">
                @method('put') 
                    @csrf
                    <!-- Tanggal Mulai -->
                    <div class="row mb-3">
                        ...
                        <div class="col-md-7 col-sm-12">
                            <input class="form-control form-control-user @error('tgl_mulai') is-invalid @enderror" type="date" id="tgl_mulai" name="tgl_mulai"
                                value="{{ old('tgl_mulai', $order->tgl_mulai) }}" required autofocus>
                            @error('tgl_mulai')
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-7 col-sm-12">
                            <input class="form-control form-control-user @error('tgl_selesai') is-invalid @enderror" type="date" id="tgl_selesai" name="tgl_selesai"
                                value="{{ old('tgl_selesai', $order->tgl_selesai) }}" required autofocus>
                            @error('tgl_selesai')
                                <div class="small text-danger">
                                    {{ $message }}
                                </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-7 col-sm-12">
                            <select name="mobil_id" id="mobil_id" class="form-select">
                                <option value="" disabled selected required>-Pilih mobil-</option>
                                @foreach ($mobils as $mobil)
                                    <option value="{{ $mobil->id }}"
                                    @if ($order->mobil_id == $mobil->id) selected @endif>{{ $mobil->merek }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
